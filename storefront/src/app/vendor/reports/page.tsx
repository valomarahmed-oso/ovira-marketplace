...
import { getVendorReport, type VendorReport } from "@/lib/reports-api";
...
export default function VendorReportsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setData(await getVendorReport(from, to));
    setLoading(false);
  }, [from, to]);
  ...
  function shareWhatsApp() {
    ...
    const lines = [
      `📊 ${t.repTitle} — ${data.from_date} → ${data.to_date}`,
      "",
      `${t.repGross}: ${data.summary.gross} ${c}`,
      `${t.repNet}: ${data.summary.net} ${c}`,
      ...
      <div className="no-print flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-medium text-ink">
            <BarChart3 className="h-6 w-6 text-blue-600" /> {t.repTitle}
          </h1>
          <p className="text-sm text-ink-400">{t.vrepSubtitle}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            {[
              { label: t.repGross, value: formatPrice(data.summary.gross, data.currency) },
              { label: t.repCommission, value: formatPrice(data.summary.commission, data.currency) },
              { label: t.repNet, value: formatPrice(data.summary.net, data.currency) },
              { label: t.repOrders, value: String(data.summary.orders) },
              { label: t.vrepUnits, value: String(data.summary.units) },
              ...
              <table className="w-full">
                ...
                <tbody>
                  {data.top_products.map((r, i) => (
                    <tr key={i} className="border-b border-line last:border-0"><td className={td}>{r.title}</td><td className={td}>{r.qty}</td><td className={td}>{formatPrice(r.revenue, data.currency)}</td></tr>
                  ))}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  ...
                  {data.low_stock.length === 0 && <tr><td className="py-3 text-sm text-ink-400" colSpan={2}>{t.repNoData}</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
