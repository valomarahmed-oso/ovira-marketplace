...
export function ProductReviews({ slug }: { slug: string }) {
  ...
  return (
    <section>
      <SectionHeading title={t.revTitle} />
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          {loading ? (
            ...
            <div className="card p-6 text-center text-sm text-ink-400">{t.revEmpty}</div>
          ) : (
            list.map((r) => (
        ...
        <div className="space-y-4">
          ...
          {user ? (
            <form onSubmit={submit} className="card space-y-3 p-5">
              <h3 className="font-medium text-ink">{t.revAddYours}</h3>
              ...
              <textarea
                ...
                placeholder={t.revBodyPlaceholder}
