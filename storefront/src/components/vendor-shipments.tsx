 ...
 * vendor creates the shipment for the items they sold and drives its status
 * (Picked Up → In Transit → Delivered). The platform aggregates: the order
 * completes once every vendor's shipment is delivered. */
export function VendorShipments({ order }: { order: string }) {
  ...
  useEffect(() => {
    let cancelled = false;
    getMyOrderShipments(order).then((s) => !cancelled && setShipments(s));
    return () => {
      cancelled = true;
    };
  }, [order]);
  ...
  async function create() {
    ...
    try {
      ...
      setShipments(await getMyOrderShipments(order));
    } catch (e) {
      setError(msg(e));
    } finally {
      setBusy(false);
  ...
  if (shipments === null) {
    return (
      <div className="flex items-center gap-2 text-sm text-ink-400">
        <Loader2 className="h-4 w-4 animate-spin text-blue-600" /> تحميل الشحن…
      </div>
    );
  }
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-xs font-medium uppercase tracking-wide text-ink-400">
          <Truck className="h-4 w-4" /> شحن طلبك
        </div>
        {shipments.length === 0 && (
          <button
            type="button"
          ...
          {shipments.map((s) => (
            <div key={s.name} className="flex flex-wrap items-center justify-between gap-2 px-3 py-2 text-sm">
              <div className="min-w-0 text-xs text-ink-400">
                ...
                {s.tracking_number && (
                  <span className="ms-2 font-tech" dir="ltr">
                    {s.tracking_number}
                  </span>
                )}
                {s.tracking_url && (
                  <a
                    ...
                    className="ms-2 inline-flex items-center gap-0.5 text-blue-600 hover:underline"
                  >
